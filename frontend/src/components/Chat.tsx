...
export function Chat() {
  ...
  const { readyState, sendJsonMessage } = useWebSocket(user ? `wss://${apiHost}/chats/${conversationName}/` : null, {
    queryParams: {
  ...
  async function fetchMessages() {
    const apiRes = await fetch(
      `https://${apiHost}/api/messages/?conversation=${conversationName}&page=${page}`,
      {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    async function fetchConversation() {
      const apiRes = await fetch(`https://${apiHost}/api/conversations/${conversationName}/`, {
        method: "GET",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  return (
    <div>
      <span>The WebSocket is currently {connectionStatus}</span> {/* закоменть потом эту строчку */}
      {
        conversation && (
          <div className="py-6">
